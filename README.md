...
- 使用 PyTorch 重新实现 YouTubeDNN 双塔召回模型
- 使用 PyTorch 重新实现 DeepFM 排序模型
- 支持 NVIDIA GPU 训练与在线推理
- 保留完整的离线训练、在线召回、排序、重排和前端展示流程
- 完善测试、项目文档和可复现部署流程

## 项目来源

原始项目：[Datawhale FunRec](https://github.com/datawhalechina/fun-rec)

本仓库以学习和非商业用途为目的，主要记录从 TensorFlow 到 PyTorch 的模型迁移及工程实践过程。
