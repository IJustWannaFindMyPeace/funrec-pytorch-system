...
- [x] 在线服务加载同一组冻结工件并验证哈希。
- [ ] 最终候选方案与 V0 完成多随机种子复验（后续优化阶段门槛，不阻塞 V0 冻结）。

## 在线同工件审计

- 宿主机训练产物、部署目录和 Docker 容器内四个核心工件的 SHA256 完全一致。
- 在线请求返回 20 个不重复电影，合并召回候选数为 100。
- 召回通道包含 `youtube_dnn` 与 `user_preference`，排序策略为 `deepfm`，HTTP 状态为 200。
- 离线导出的物品向量形状为 `(3883, 16)`；在线矩阵为 `(3884, 16)`，新增行是索引 0 的全零 padding/OOV 向量，真实电影向量从索引 1 开始，不是数据数量不一致。
- 机器可读证据：`docs/results/baseline_v0_online_verification.json`。
